...
private:
    void cameraInfoCallback(const sensor_msgs::msg::CameraInfo::SharedPtr msg) {camera_info_ = msg;}
...
YoloDispatcher::YoloDispatcher() : Node("yolo_dispatcher") {
    ...
    camera_info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(camera_info_topic, 10, std::bind(&YoloDispatcher::cameraInfoCallback, this, _1));
...
void YoloDispatcher::yoloCallback(dynamic_nav_msgs::msg::YoloData::SharedPtr msg) {
    ...
    // filter depth for rtabmap and obstacles_tracker
    auto static_depth = msg->depth; // depth only if mask pixel is 0
    ...
    msg->mask.header.stamp = msg->rgb.header.stamp;
    ...
    detector_data.mask = *mask->toImageMsg().get();
